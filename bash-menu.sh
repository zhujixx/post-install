...
if [ "$BASH_SOURCE" = "" ]; then
  echo "ERROR: Script requires to be running under bash"
  exit 1
fi
...
menu_HandleInput() {
  ...
  if [[ $key = $upKey ]] || [[ $key = $leftKey ]]; then
    ...
    return $after
  elif [[ $key = $homeKey ]] || [[ $key = $home2Key ]]; then
    return 0
  elif [[ $key = $endKey ]] || [[ $key = $end2Key ]]; then
    return $menuLastItem
    ...
    # See if we wanrt to jump to a menu item by entering the first character
